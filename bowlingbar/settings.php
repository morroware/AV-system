...
try {
    $fileOwner = function_exists('posix_getpwuid') ? posix_getpwuid(fileowner($configFile)) : ['name' => 'unknown'];
...
$backupFiles = [];
$backupFilesPattern = __DIR__ . '/config_backup_*.php';
if ($handle = @glob($backupFilesPattern)) {
    ...
    // Take just the first 10 (latest) files
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['restore_backup'])) {
    try {
        ...
        $backups = [];
        if ($handle = @glob(__DIR__ . '/config_backup_*.php')) {
            // Sort files by modification time
            usort($handle, function($a, $b) {
                return filemtime($b) - filemtime($a);
            });
        ...
        // Remove all but the first 3 files
